...
- **AI-объяснение на русском:** улучшения, оставшиеся проблемы и компромиссы. В интерфейсе доступны `gpt-4o-mini`, `gpt-4.1` и модель из настройки окружения.
...
| **`gpt-4.1`** | Дополнительный вариант выбора в интерфейсе |
...
Для живого AI нужны интернет и ключ с доступом к выбранной модели. После изменения `.env` перезапустите приложение. Модель для следующего запроса можно переключить непосредственно в интерфейсе.
...
| Изменения Нуры | S1 **+10**, S2 **+8.75**, B1 **+12.5** |
| Изменение Сарыарки | E2 **+8.75** |
...
- **Отказ AI:** включите «Показать работу без AI» и снова запросите объяснение. API не вызывается; Score и три таблицы остаются, текст подписан как резервный. После проверки снимите отметку.
...
Для проверки реальных запросов к двум моделям:
